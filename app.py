...
with st.sidebar:
    ...
    ministry = st.text_input("Министерство / ведомство", key="ministry",
        value="Министерство науки и высшего образования Российской Федерации")
    org = st.text_input("Организация", key="org",
        value="ФГБОУ ВО «Название университета»")
    title = st.text_input("Название НИР", key="title",
        value="Исследование методов машинного обучения")
    theme_code = st.text_input("Шифр темы", key="theme_code", value="НИР-2024-01")
    udc = st.text_input("УДК", key="udc", value="004.8")
    inv_number = st.text_input("Инв. №", key="inv_number", value="")
    city = st.text_input("Город", key="city", value="Москва")
    year = st.text_input("Год", key="year", value="2024")

    st.divider()
    st.subheader("👤 Руководитель")
    head_position = st.text_input("Должность руководителя", key="head_position", value="д-р техн. наук, проф.")
    head_name     = st.text_input("ФИО руководителя", key="head_name", value="И.О. Фамилия")
    approver_name = st.text_input("ФИО утверждающего", key="approver_name", value="И.О. Фамилия")
    approver_position = st.text_input("Должность утверждающего", key="approver_position", value="Директор")
    ...
    add_title   = st.checkbox("Генерировать титульный лист", key="add_title", value=True)
    add_abstract = st.checkbox("Добавить заготовку реферата", key="add_abstract", value=True)
    add_toc     = st.checkbox("Автоматическое содержание (TOC)", key="add_toc", value=True)
